...
int CNaviMemory::CheckMaxCpu()
{
	...
	for (int i = 0; i < m_vtModuleMemory.size(); i++)
	{
		...
		if (pModule)
		{
			if (iRet < pModule->CheckCpu())
			{
				iRet = pModule->CheckCpu();
				modName = pModule->GetModuleName();
			}
		}
	}

	std::cout << " Max Cpu : " << modName << " -->" << iRet << std::endl;
...
int CNaviMemory::CheckCpu()
{
	...
	std::cout << " Navi Cpu : " << iRet << std::endl;
